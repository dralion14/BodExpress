...
namespace BodExpress
{
    ...
    {
        public const string URL_LOGIN = "~/LogOn.aspx";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack && !Page.IsCallback)
            {
                ...
                if (ActiveUser != null)
                {
                    ...
                    //this.generaMenu();
                    if (!TienePermiso(Request.AppRelativeCurrentExecutionFilePath,ActiveUser.area))
                    {
                        //Si no tiene permiso se redirige
                        //Response.Redirect(login.URL_DEFECTO, true);
                        //Pagina Pag = UsuarioCRUD.getPagina(Request.AppRelativeCurrentExecutionFilePath);
                        //if (Pag == null)
                        //    PaginaSinPermiso = "S/I";
                        //else
                        //    PaginaSinPermiso = Pag.Text;
        ...
        public bool TienePermiso(string urlConsulta, string area)
        {
            return true;
        }
    }
